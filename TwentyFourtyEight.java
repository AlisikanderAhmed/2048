...
public class TwentyFourtyEight extends JFrame implements ActionListener, KeyListener{
	...
	public TwentyFourtyEight() throws IOException   {
		...
		final ImageIcon info = new ImageIcon("info.png");
		final ImageIcon icon = new ImageIcon("icon.png");

		name = (String) JOptionPane.showInputDialog(null, "What is your name?", "Welcome", JOptionPane.INFORMATION_MESSAGE, info, null, "");
		JOptionPane.showMessageDialog(null, "\tHello " + name + '!' + "\n\nHOW TO PLAY: Use your arrow keys to move the tiles. \nWhen two tiles with the same number touch, they merge into one!\n\nHit 2048 to WIN!", "Welcome", JOptionPane.INFORMATION_MESSAGE, icon);
	}

	public void displayBoard(){	//Display Board function to set Colours
	...
	public boolean pressedDown(){	//Function for moving Board Down if user Presses Down
		boolean moved = false;
	...
	public boolean pressedUp(){ //Function for moving Board Up if user Presses Up
		boolean moved = false;
	...
	public boolean pressedLeft(){ //Function for moving Board Left if user Presses Left
		boolean moved = false;
		for (int cols=0; cols<3; cols++){
			for (int rows=0; rows<4; rows++){
	...
	public boolean pressedRight(){ //Function for moving Board Right if user Presses Right
		boolean moved = false;
	...
	public void spawnNew(){	//Function for resetting and reloading board
		spawningVar = true;
	...
	public void writeFile() {	//Write File Function to write highscores and Top Scores.
		try {
	...
	public static void myDisplay(ArrayList<Integer> topScores){	//Display function to display scores
	...
	public void checkWinLose(){	//Scan board to check if User hits 2048
		for (int cols=0; cols<4; cols++){
			for (int rows=0; rows<4; rows++)
	...
	public void youWin(){		//A win function to set the board letters to 'YOU WIN' and change background colour
		writeFile();
	...
	public void youLose(){	//A Lose function to set the board letters to 'YOU LOSE' and change background colour
		playing = false;
		writeFile();
		for (int cols=0; cols<4; cols++)
			for (int rows=0; rows<4; rows++)
	...
	public void newGameSpawn(){	//Spawn function to randomly load board
		xRan = (int) (Math.random()*4);
		yRan = (int) (Math.random()*4);
	...
	public void newGame(){	//New game function to set board to defaults
		for (int cols=0; cols<4; cols++)
			for (int rows=0; rows<4; rows++)
				board[rows][cols] = 0;
		score=0;
	...
	public void startGame(){	//Inital Start Game function to set buttons and do inital start
	...
	public static void main( String args[] ) throws IOException {	//Main
		TwentyFourtyEight app = new TwentyFourtyEight();
		app.setLocationRelativeTo(null);
	}

	public void keyPressed(KeyEvent e){	//Key Pressed Key Events
